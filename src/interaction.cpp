...
Interaction::Interaction(const std::string &_interactionName, State *_state)
{
  ...
  ASSERT(_state != NULL, "no state given in Interaction::Interaction()");
  *this = Interaction(Interaction::getInteractionDataTree(_interactionName), _state);
...
Interaction::Interaction(DataTree _dataTree, State *_state)
{
  ...
  interactionName = "";
  dataTree.get(interactionName, "interaction/name", true);
  ASSERT(interactionName != "", "no interaction specified");
  ...
  if (dataTree.getB("interaction/spinOrbitFR"    , false)) addField("spinOrbitFR"    );
  if (dataTree.getB("interaction/tensorFR"       , false)) addField("tensorFR"       );
...
coulombExact        True
  ...
  ERROR("Invalid interaction type: '" + interactionName + "'");
