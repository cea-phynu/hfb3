...
TEST(Meta, energy_D1S_1ct)
{
  ...
  ASSERT_NEAR(interaction("kinetic"         , 0)->energy(NEUTRON, Field::DIRECT  ),   369.860570, err);
  ASSERT_NEAR(interaction("coulomb (Slater)", 0)->energy(NEUTRON, Field::DIRECT  ),     0.000000, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(NEUTRON, Field::DIRECT  ), -1001.729770, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(NEUTRON, Field::DIRECT  ),   -44.636064, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(NEUTRON, Field::EXCHANGE),   456.097490, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(NEUTRON, Field::EXCHANGE),  -527.734172, err);
  ASSERT_NEAR(interaction("spin-orbit"      , 0)->energy(NEUTRON, Field::DIRECT  ),    -7.100356, err);
  ASSERT_NEAR(interaction("density"         , 0)->energy(NEUTRON, Field::DIRECT  ),   552.709224, err);
  ...
  ASSERT_NEAR(interaction("central"         , 0)->energy(NEUTRON, Field::PAIRING ),     4.492866, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(NEUTRON, Field::PAIRING ),    -8.902653, err);
  ASSERT_NEAR(interaction("rearrangement"   , 0)->energy(NEUTRON, Field::DIRECT  ),    96.139861, err);
  ASSERT_NEAR(interaction("kinetic"         , 0)->energy(PROTON , Field::DIRECT  ),   309.733471, err);
  ASSERT_NEAR(interaction("coulomb (Slater)", 0)->energy(PROTON , Field::DIRECT  ),    79.044426, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(PROTON , Field::DIRECT  ), -1009.472095, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(PROTON , Field::DIRECT  ),   -21.056040, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(PROTON , Field::EXCHANGE),   449.152343, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(PROTON , Field::EXCHANGE),  -514.596065, err);
  ASSERT_NEAR(interaction("spin-orbit"      , 0)->energy(PROTON , Field::DIRECT  ),    -1.675028, err);
  ASSERT_NEAR(interaction("density"         , 0)->energy(PROTON , Field::DIRECT  ),   552.709224, err);
  ...
  ASSERT_NEAR(interaction("rearrangement"   , 0)->energy(PROTON , Field::DIRECT  ),    88.095728, err);
}
...
TEST(Meta, energy_D1S_2ct)
{
  ...
  ASSERT_NEAR(interaction("kinetic"         , 0)->energy(NEUTRON, Field::DIRECT  ),   370.706224, err);
  ASSERT_NEAR(interaction("coulomb (Slater)", 0)->energy(NEUTRON, Field::DIRECT  ),     0.000000, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(NEUTRON, Field::DIRECT  ), -1007.123990, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(NEUTRON, Field::DIRECT  ),   -44.992915, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(NEUTRON, Field::EXCHANGE),   458.164246, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(NEUTRON, Field::EXCHANGE),  -529.498325, err);
  ASSERT_NEAR(interaction("spin-orbit"      , 0)->energy(NEUTRON, Field::DIRECT  ),    -7.154425, err);
  ASSERT_NEAR(interaction("density"         , 0)->energy(NEUTRON, Field::DIRECT  ),   557.056243, err);
  ...
  ASSERT_NEAR(interaction("rearrangement"   , 0)->energy(NEUTRON, Field::DIRECT  ),    96.940615, err);
  ASSERT_NEAR(interaction("kinetic"         , 0)->energy(PROTON , Field::DIRECT  ),   310.321577, err);
  ASSERT_NEAR(interaction("coulomb (Slater)", 0)->energy(PROTON , Field::DIRECT  ),    79.170771, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(PROTON , Field::DIRECT  ), -1014.983015, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(PROTON , Field::DIRECT  ),   -21.069888, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(PROTON , Field::EXCHANGE),   451.122572, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(PROTON , Field::EXCHANGE),  -516.202920, err);
  ASSERT_NEAR(interaction("spin-orbit"      , 0)->energy(PROTON , Field::DIRECT  ),    -1.685195, err);
  ASSERT_NEAR(interaction("density"         , 0)->energy(PROTON , Field::DIRECT  ),   557.056245, err);
  ...
  ASSERT_NEAR(interaction("central"         , 0)->energy(PROTON , Field::PAIRING ),     0.000000, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(PROTON , Field::PAIRING ),    -0.000000, err);
  ASSERT_NEAR(interaction("rearrangement"   , 0)->energy(PROTON , Field::DIRECT  ),    88.744049, err);
}
...
TEST(Meta, energy_D12S_1ct)
{
  ...
  ASSERT_NEAR(interaction("central"         , 0)->energy(NEUTRON, Field::PAIRING ),     4.492866, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(NEUTRON, Field::PAIRING ),    -8.902653, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(NEUTRON, Field::DIRECT  ),  1155.987358, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(NEUTRON, Field::EXCHANGE),  -603.282131, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(NEUTRON, Field::PAIRING ),     0.000000, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(NEUTRON, Field::DIRECT  ),   192.664560, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(NEUTRON, Field::EXCHANGE),   -96.524951, err);
  ...
  ASSERT_NEAR(interaction("central"         , 0)->energy(PROTON , Field::PAIRING ),     0.000000, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(PROTON , Field::PAIRING ),    -0.000000, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(PROTON , Field::DIRECT  ),  1059.246031, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(PROTON , Field::EXCHANGE),  -506.540804, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(PROTON , Field::PAIRING ),     0.000000, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(PROTON , Field::DIRECT  ),   176.541005, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(PROTON , Field::EXCHANGE),   -88.445538, err);
...
TEST(Meta, energy_D12S_2ct)
{
  ...
  ASSERT_NEAR(interaction("central"         , 0)->energy(NEUTRON, Field::PAIRING ),     4.329000, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(NEUTRON, Field::PAIRING ),    -8.604881, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(NEUTRON, Field::DIRECT  ),  1165.671644, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(NEUTRON, Field::EXCHANGE),  -608.619862, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(NEUTRON, Field::PAIRING ),     0.000000, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(NEUTRON, Field::DIRECT  ),   194.278607, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(NEUTRON, Field::EXCHANGE),   -97.338047, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(NEUTRON, Field::PAIRING ),     0.000000, err);
  ASSERT_NEAR(interaction("kinetic"         , 0)->energy(PROTON , Field::DIRECT  ),   310.321577, err);
  ASSERT_NEAR(interaction("coulomb (Slater)", 0)->energy(PROTON , Field::DIRECT  ),    79.170771, err);
  ASSERT_NEAR(interaction("central"         , 0)->energy(PROTON , Field::DIRECT  ), -1014.983015, err);
  ...
  ASSERT_NEAR(interaction("central"         , 0)->energy(PROTON , Field::PAIRING ),     0.000000, err);
  ASSERT_NEAR(interaction("central"         , 1)->energy(PROTON , Field::PAIRING ),    -0.000000, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(PROTON , Field::DIRECT  ),  1067.084212, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(PROTON , Field::EXCHANGE),  -510.032429, err);
  ASSERT_NEAR(interaction("density FR"      , 0)->energy(PROTON , Field::PAIRING ),     0.000000, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(PROTON , Field::DIRECT  ),   177.847369, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(PROTON , Field::EXCHANGE),   -89.104002, err);
  ASSERT_NEAR(interaction("rearrangement FR", 0)->energy(PROTON , Field::PAIRING ),     0.000000, err);
}
